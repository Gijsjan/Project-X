define([
	...
	return {
		content_objects: ['note', 'video', 'link', 'document', 'event'],
		objects: ['user', 'tag', 'comment', 'country', 'group'],
		...
		*/
		...
		getParentType: function(object_type) {
			return ($.inArray(object_type, this.content_objects) > -1) ? 'content' : 'object';
		},
		...
		renderTemplate: function(template_type, model) {
			var template_id = '#tpl' + template_type + this.ucfirst(model.get('type'));
